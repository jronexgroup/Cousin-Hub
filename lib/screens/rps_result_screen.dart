...
class RpsResultScreen extends StatefulWidget {
  final String roomId;
  const RpsResultScreen({super.key, required this.roomId});
  @override
...
class _RpsResultScreenState extends State<RpsResultScreen> {
  ...
  void initState() {
    super.initState();
    _db.ref('rpsRooms/${widget.roomId}').once().then((s) {
  ...
  String get _winnerName {
    final wid = _room['winner'];
    final players = _room['players'] as Map? ?? {};
    if (wid == null || !players.containsKey(wid)) return 'No one';
    return (players[wid] as Map)['name'] ?? 'Cousin';
  }

  List<Map<String, dynamic>> get _scores {
