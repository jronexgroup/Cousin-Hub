...
import 'spy_chat_game_screen.dart';
import 'spy_chat_models.dart';

class SpyChatLobbyScreen extends StatefulWidget {
  final String? roomId;
  final bool isHost;
  const SpyChatLobbyScreen({super.key, this.roomId, this.isHost = true});

  @override
  State<SpyChatLobbyScreen> createState() => _SpyChatLobbyState();
}

class _SpyChatLobbyState extends State<SpyChatLobbyScreen> {
  ...
  Future<void> _joinRoom() async {
    ...
    await _db.ref('spyChatRooms/$_roomId/players/$_myUid').set({
      'name': _myName, 'photo': _myPhoto, 'isReady': false, 'alive': true,
    });
    _listenRoom();
    if (mounted) setState(() => _loading = false);
  }
  ...
  void _listenRoom() {
    if (_roomId == null) return;
    _sub = _db.ref('spyChatRooms/$_roomId').onValue.listen((e) {
      ...
      if (st == 'wordReveal' && _roomId != null) {
        _sub?.cancel();
        Navigator.pushReplacement(context, MaterialPageRoute(
          builder: (_) => SpyChatGameScreen(
  ...
  Future<void> _createRoom() async {
    if (_selected.length < kMinPlayers - 1) return;
    setState(() => _creating = true);
    _roomId = 'spy_${DateTime.now().millisecondsSinceEpoch}';

    final players = <String, dynamic>{
      _myUid: {'name': _myName, 'photo': _myPhoto, 'isReady': false, 'alive': true},
    };
    for (final cuid in _selected) {
      ...
      players[cuid] = {
        ...
        'photo': u['photoUrl'] ?? '',
        'isReady': false, 'alive': true,
      };
    }

    await _db.ref('spyChatRooms/$_roomId').set({
      'hostUid': _myUid, 'hostName': _myName, 'status': 'waiting',
      'players': players,
    });

    for (final cuid in _selected) {
      await _db.ref('spyChatInvites/$cuid/$_roomId').set({
      ...
      if (tk.exists) {
        await _db.ref('notifications').push().set({
          'toToken': tk.value,
          'title': '🕵️ $_myName invited you to Spy Chat!',
  ...
  Future<void> _toggleReady() async {
    if (_roomId == null) return;
    _myReady = !_myReady;
    await _db.ref('spyChatRooms/$_roomId/players/$_myUid/isReady').set(_myReady);
  }

  Future<void> _startGame() async {
    if (_roomId == null) return;
    await _db.ref('spyChatRooms/$_roomId/status').set('wordReveal');
  }
  ...
  bool get _allReady => _players.length >= kMinPlayers && _players.every((p) => p['isReady'] == true);
  ...
  Widget _buildPicker() {
    final canCreate = _selected.length >= kMinPlayers - 1;
    return Column(children: [
      Container(
        ...
        child: Text(
          canCreate
              ? 'Tap Create Room to invite selected cousins'
              : 'Select ${kMinPlayers - 1 - _selected.length} more cousin${_selected.length >= kMinPlayers - 1 ? '' : 's'} (need $kMinPlayers\u2013$kMaxPlayers)',
      ...
      Container(
        ...
        child: SafeArea(child: AppTheme.gradientButton(
          label: canCreate
              ? '🕵️ Create Room (${_selected.length + 1}/$kMinPlayers\u2013$kMaxPlayers)'
              : '👆 Select ${kMinPlayers - 1 - _selected.length} more',
          loading: _creating,
  ...
  Widget _buildLobby() {
    ...
    return Column(children: [
        ...
        itemBuilder: (_, i) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(14),
              border: Border.all(color: ready ? AppTheme.primary : Colors.grey.shade800, width: ready ? 2 : 1)),
            child: Row(children: [
              Container(width: 42, height: 42,
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(ready ? '✅ Ready' : '⏳ Not ready',
                  style: TextStyle(fontSize: 11, color: ready ? Colors.greenAccent : Colors.white38)),
              ])),
              if (isMe)
                GestureDetector(
                  onTap: _toggleReady,
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: _myReady ? Colors.green.shade700 : Colors.grey.shade700,
                      borderRadius: BorderRadius.circular(20)),
                    child: Text(_myReady ? 'Ready' : 'Ready?',
      ...
      Container(
        padding: const EdgeInsets.fromLTRB(12, 8, 12, 16),
        color: const Color(0xFF0A0A1A),
        child: SafeArea(child: isHost
            ? AppTheme.gradientButton(
                label: _allReady
                    ? '🕵️ Start Game (${spyCount(_players.length)} spy${spyCount(_players.length) > 1 ? 's' : ''})'
                    : _players.length < kMinPlayers
                        ? 'Waiting for ${kMinPlayers - _players.length} more...'
                        : 'Waiting for all to ready up...',
                onTap: _allReady ? _startGame : null, height: 52)
            : Container(
