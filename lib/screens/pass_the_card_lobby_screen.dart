...
import 'pass_the_card_selection_screen.dart';

class PassTheCardLobbyScreen extends StatefulWidget {
  final String? roomId;
  final bool isHost;
  const PassTheCardLobbyScreen({super.key, this.roomId, this.isHost = true});

  @override
  State<PassTheCardLobbyScreen> createState() => _PassTheCardLobbyState();
}

class _PassTheCardLobbyState extends State<PassTheCardLobbyScreen> {
  ...
  Future<void> _joinRoom() async {
    ...
    await _db.ref('passTheCardRooms/$_roomId/players/$_myUid').set({
      'name': _myName, 'photo': _myPhoto,
    });
    _listenRoom();
    if (mounted) setState(() => _loading = false);
  }
  ...
  void _listenRoom() {
    if (_roomId == null) return;
    _sub = _db.ref('passTheCardRooms/$_roomId').onValue.listen((e) {
      ...
      if (st == 'selecting' && _roomId != null) {
        _sub?.cancel();
        Navigator.pushReplacement(context, MaterialPageRoute(
          builder: (_) => PassTheCardSelectionScreen(
  ...
  Future<void> _createRoom() async {
    ...
    _roomId = 'ptc_${DateTime.now().millisecondsSinceEpoch}';

    final players = <String, dynamic>{
      _myUid: {'name': _myName, 'photo': _myPhoto},
    };
    ...
    await _db.ref('passTheCardRooms/$_roomId').set({
      'hostUid': _myUid, 'hostName': _myName, 'status': 'waiting',
      'players': players,
    });

    for (final cuid in _selected) {
      await _db.ref('passTheCardInvites/$cuid/$_roomId').set({
      ...
      if (tk.exists) {
        await _db.ref('notifications').push().set({
          'toToken': tk.value,
          'title': '🃏 $_myName invited you to Pass The Card!',
  ...
  Future<void> _startGame() async {
    if (_roomId == null) return;
    await _db.ref('passTheCardRooms/$_roomId/status').set('selecting');
  }
      ...
      Container(
        ...
        child: Text(
          _selected.length >= 3
              ...
              : 'Select ${3 - _selected.length} more cousin${_selected.length >= 3 ? '' : 's'} (need exactly 4 players)',
          ...
          : ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return GestureDetector(
                  onTap: _selected.length >= 3 && !sel ? null
                      : () => setState(() => sel ? _selected.remove(uid) : _selected.add(uid)),
      ...
      Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${_players.length}/4 players joined',
